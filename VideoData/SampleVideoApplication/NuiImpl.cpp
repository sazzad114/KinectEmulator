...
FileReader fio("video.dat");
unsigned long packetCount = 0;

byte readArr[1228800];
byte subFrame[1228800];
byte keyFrame[1228800];
...
void CSkeletalViewerApp::Nui_GotVideoAlert( )
{
	...
	long size,destSize;
	fio.fp.read((char*)&size,sizeof(long));
	fio.fp.read((char*)readArr,size);
	//if(packetCount%3 == 1) {
		
	for(int i=0;i<1228800;i++){
		keyFrame[i] = readArr[i];
	}

	  m_DrawVideo.DrawFrame( (BYTE*)keyFrame );
		
	/*}
	else
	{
		CLZWCompression decompress;
		decompress.Decompress(readArr, subFrame, size, &destSize);
		for(int i=0;i<1228800;i++)
		{
			subFrame[i] = subFrame[i]+keyFrame[i];	
		}
		m_DrawVideo.DrawFrame( (BYTE*)subFrame );
	}*/
